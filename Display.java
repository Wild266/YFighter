...
public class Display extends JPanel implements KeyListener
{
   ...
   private double g;                          // the force of gravity
   ...
   public void setBG(String m){
      try {
         backgroundImage = ImageIO.read(new File(m)); //sets the background image to "space.jpg"
      } 
      catch (Exception e) {
         System.out.println("Image not found");                 //there is no "space.jpg"
      }
   ...
   */
   public void paint(Graphics g)
   {
      ...
      if(mode == 0)
      {
         g.setColor(new Color(204, 131,239));                                   
         g.setFont(new Font("Copperplate Gothic Bold", Font.BOLD, 70));
         g.drawString("Space Table",37,300);                              //Draws Title in magenta in font specified
         
         
         g.setColor(Color.gray);
         g.setFont(new Font("Impact", Font.ITALIC, 20));
         g.drawString("Press space to start",700,550);                   //Draws prompt to start specification in gray in font specified
      }
      else if(mode == 1)
      {

         g.setColor(Color.white);
         g.fillRect(100,yMin, 750, 10);                                        //rectangle platform is drawn
      ...
      }else if(mode == 2)
      {
         g.setFont(new Font("Agency FB", Font.BOLD, 30));                        
          g.setColor(Color.white);
          g.drawString("Developed By Joseph Rother, Akshan Sameullah, and Praise Apanisile",270,570);
          //draws author label with color white and specified font
         if((dFs[0].getHealth() <= 0 || dFs[0].getPos()[1] >= 600)&&(dFs[1].getHealth() <= 0 || dFs[1].getPos()[1] >= 600)){
            g.setFont(new Font("Agency FB", Font.BOLD, 190));
            g.setColor(Color.green);
            g.drawString("It's a",500,200);
            g.drawString("Tie",500,400);   
         }
         else if(dFs[1].getHealth() <= 0 || dFs[1].getPos()[1] >= 600)
         {
            g.setFont(new Font("Agency FB", Font.BOLD, 190));
            g.setColor(Color.red);
         ...
         Graphics2D g2 = (Graphics2D) g;
            ...
            g2.setFont(new Font("Magneto", Font.BOLD, 130));
   ...
   */
   public Fighter[] loadGame(Platform p)
   {
      ...
      fs[0] = new Fighter(totalH,100,10,s);     //makes a fighter at 0,10
      fs[1] = new Fighter(totalH,800,10,s);  //makes a fighter at 1000,10
      return fs;
   }
   ...
   */
   public void setVels(Fighter f1,boolean[] inputs, Platform plat)
   {
      ...
      } else if (inputs[2] == true )//&& f1.getPos()[1] == plat.getY()- f1.getSize())
      {
         //set the upward momentum to the jump force
         f1.setMvmtVel(f1.getMvmtVel()[0],  j);
      }
   ...
   */
   ...
   {
      if((f1.getPos()[0]>50)&&(f1.getPos()[0]<850))
